...
class HumanLikeHeadingExtractor:
    ...
    def _is_valid_heading_text(self, block: TextBlock) -> bool:
        ...
        if not text:
            return False
        if re.match(r'^[\•\-\–\—\*\d\.]\s', text): # Checks for bullets, dashes, numbers.
            return False
        # Remove common heading prefixes/suffixes for analysis
        clean_text = re.sub(r'^\d+\.?\s*', '', text)  # Remove leading numbers like "1. " or "3.1 "
        ...
        non_heading_patterns = [
            ...
            r'^\d+\s+\d+\s*$',  # Two numbers
            r'^\d+\s+\d+\s+\d+\s*$',  # Three numbers
