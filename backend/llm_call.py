...
def get_llm_response(messages):
    ...
    if provider == "gemini":
        ...
        if api_key:
            # Prefer explicit API key over ADC; clear env that forces ADC if present
...
def get_llm_response_stream(messages):
    """Yield text chunks from the configured LLM provider. Falls back to a single-shot response if streaming is not available."""
    provider = os.getenv("LLM_PROVIDER", "gemini").lower()

    def extract_text(chunk) -> str:
        # LangChain Chat* .stream yields AIMessageChunk-like objects with .content
        try:
            text = getattr(chunk, "content", None)
            if isinstance(text, str):
                return text
            # Some providers may put text in .message.content or .delta
    ...
    try:
        if provider == "gemini":
            ...
            if not all([api_key, api_base, api_version]):
                raise ValueError("Missing one of AZURE_OPENAI_KEY, AZURE_OPENAI_BASE, or AZURE_API_VERSION.")
            llm = AzureChatOpenAI(azure_deployment=deployment_name, openai_api_version=api_version, azure_endpoint=api_base, api_key=api_key, temperature=0.7)
            for chunk in llm.stream(messages):
                t = extract_text(chunk)
                if t:
                    yield t
            ...
            # ChatOllama supports streaming; iterate chunks
            for chunk in llm.stream(messages):
                t = extract_text(chunk)
                if t:
                    yield t
        ...
        try:
            full = get_llm_response(messages)
            if full:
                yield str(full)
        except Exception as e:
            yield f"[ERROR] {e}"
